...
char	theChar;
	...
	{
	#if (FLASHEND > 0x10000)
		theChar	=	pgm_read_byte_far((uint32_t)(uint16_t)dataPtr + ii);
	#else
		theChar	=	pgm_read_byte_near((uint16_t)dataPtr + ii);
	#endif
		if (theChar != 0)
		{
			sendchar(theChar);
...
int		errorCount;
	...
	ii			=	0;
#if (FLASHEND > 0x10000)
	while (((theChar = pgm_read_byte_far((uint32_t)(uint16_t)gTextMsg_Explorer + ii)) != '*') && (ii < 512))
#else
...
unsigned long	absoluteAddr;
	...
	while (vectorIndex < kInterruptVectorCount)
	{
		...
		sendchar(0x20);
	#if (FLASHEND > 0x10000)
		stringPointer	=	pgm_read_word_far((uint32_t)(uint16_t)&(gInterruptNameTable[vectorIndex]));
	#else
		stringPointer	=	pgm_read_word_near((uint16_t)&(gInterruptNameTable[vectorIndex]));
	#endif
		PrintFromPROGMEM((char *)(uint16_t)stringPointer, 0);
	#endif
		PrintNewLine();

...
#define analogPinToChannel(p) \
	((p) < NUM_ANALOG_INPUTS ? (NUM_ANALOG_INPUTS - 1) - (p) : -1) // Correct analogue pin numbering
...
const uint16_t __pcmsk[] = {(uint16_t)&PCMSK0, (uint16_t)&PCMSK1, (uint16_t)&PCMSK2,
							(uint16_t)&PCMSK3};
...
const uint16_t PROGMEM port_to_input_PGM[] = {
	NOT_A_PORT, (uint16_t)&PINA, (uint16_t)&PINB, (uint16_t)&PINC, (uint16_t)&PIND,
};
